...
def exclude_ur(binary_image, contour, point):
    ...
    if ret > 0:
        ...
        # cv2.circle(binary_image, point, 5, (0, 255, 0), 2)  # 在图像上用绿色圆圈标记内部点
        return True
    elif ret == 0:
        ...
        # cv2.circle(maize_image, point, 5, (0, 0, 255), 1)  # 小圆，填充
        return False
...
def get_white_line(contour):
    ...
    # cv2.drawContours(maize_image, [hull], -1, (0, 255, 255), 3)  # 红色表示凸包
    # cv2.polylines(maize_image, [hull], True, (255, 255, 255), 5)  #用这个
    # 计算轮廓的质心
    ...
    for point in hull:
        x, y = point[0]
        # cv2.circle(maize_image, point[0], 5, (0, 0, 255), 8)  # 小圆，填充
        # cv2.circle(maize_image, (x, y), 5, (0, 0, 255), 8)  # 小圆，填充！！！
        # cv2.line(maize_image, (x, y), (cX, cY), (255, 0, 0), 5)

    # 1.1 计算轮廓上所有点到质心的距离，计算平均距离作为半径，已废弃
    distances = [cv2.norm(cv2.subtract(hull, (cX, cY))) for hull in hull.squeeze()]
...
def get_line_rgb(arc_step, radius):
    ...
    for theta in np.arange(0, 2 * np.pi, arc_step):
        ...
        # 确保点在图像范围内
        # if 0 <= x < maize_image.shape[1] and 0 <= y < maize_image.shape[0]:
        if exclude_ur(maize_image, contour, scan_point):
            # cv2.circle(maize_image, scan_point, 5, (0, 0, 255), 1)   # 小圆，填充
            # 提取当前点的RGB值
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(input_folder):
        ...
        if file_ext in supported_ext:
            ...
            # 显示玉米乳熟线标记图片
            # cv2.imshow('Maturity of Maize', maize_image)
            # cv2.imshow('Maturity of Maize —— Grey', image_gray)
